...
export function ResourceList({ resources, onEdit, onDelete }: ResourceListProps) {
  return <div className="resource-list">{resources.map((resource) => <article className="resource-card" key={resource.id}>
    ...
    <div className="resource-card__footer"><span>Added {new Date(resource.createdAt).toLocaleDateString()}</span><div><a className="text-button" href="#dependency-graph">Graph</a><button className="text-button" type="button" onClick={() => onEdit(resource)}>Edit</button><button className="text-button text-button--danger" type="button" onClick={() => onDelete(resource)}>Delete</button></div></div>
  </article>)}</div>;
}