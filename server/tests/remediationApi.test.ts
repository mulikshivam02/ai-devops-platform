...
describe('remediation API routes', () => {
  ...
  it('supports create, list, detail, patch, and lifecycle routes', async () => {
    const payload = { changeId: remediation.changeId, resourceId: remediation.resourceId, type: 'configuration', title: 'Update replicas', description: 'Structured proposal', reason: 'Evidence-backed', proposedChanges: remediation.proposedChanges, evidenceIds: remediation.evidenceIds, createdBy: 'test' };
    ...
    for (const path of ['analyze', 'generate', 'validate', 'prepare-pr', 'approve', 'reject', 'dry-run', 'verify']) expect((await request(app).post(`/api/remediations/${remediation.id}/${path}`).send({ actor: 'operator', reason: 'Reviewed evidence' })).status).toBe(200);
  });
