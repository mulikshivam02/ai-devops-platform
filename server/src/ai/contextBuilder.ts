...
export async function buildInvestigationContext(changeId: string): Promise<InvestigationContext> {
  ...
  const [analysis, evidenceDocs, prediction, observation, comparison, history, securityDocs] = await Promise.all([
