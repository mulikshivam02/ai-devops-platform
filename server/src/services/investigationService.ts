...
export async function investigateChange(changeId: string) {
  ...
  try {
    const result = validateAIResult(await provider.analyze({ type: 'change_analysis', context, prompt }), allowedEvidence); return await persistCompleted(change, result, provider.name);
  } catch (error) {
    ...
    await InvestigationModel.create({ changeId: change._id, resourceId: change.resourceId, type: 'change_analysis', status: 'failed', hypotheses: [], reasoning: [], recommendations: [], limitations: ['Investigation did not complete.'], provider: provider.name, model: process.env.OLLAMA_MODEL ?? 'mock', promptVersion, errorCode: providerError instanceof AIProviderError ? providerError.code : 'AI_VALIDATION_ERROR', errorMessage: providerError.message });
    if (providerError instanceof AIProviderError) throw new AppError(503, providerError.message); throw providerError;
  }
}

async function persistCompleted(change: { _id: Types.ObjectId; resourceId: Types.ObjectId }, result: AIAnalysisResult, provider: string) {
  ...
  const doc = await InvestigationModel.create(data); return toInvestigationDTO(doc);
}
